...
export default function PublishTierPage() {
  ...
  useEffect(() => {
    if (!form.getValues('name')) redirect(`/console/communities/${community.id}/goals/create`)
  }, [])
  const onModalClose = () => {
    router.push(`/console/communities/${community.id}/goals`)
  }
  return (
    <CenterLayout
      ...
      maxW="630px"
    >
      <VStack maxW="630px" h="full" flexGrow={1} justify={{ base: 'space-between', md: 'start' }} mx="auto" gap="12" w="full">
        <VStack w="full" align="start">
          <Text fontWeight="bold" fontSize="22px" pb="3" color="#343333" display={{ base: 'block', md: 'none' }}>
            Preview
          </Text>{' '}
          <GoalCard community={community} goal={form.getValues()} format="preview" btnText={`Help grow a tier`} />
        </VStack>
        <Button onClick={() => createUpdateGoal(form.getValues())} type="submit" w="full" size="lg" colorScheme="primary" variant="solid" isLoading={isLoading}>
          Publish now
        </Button>
        <FirstGoalModal isOpen={showFirstGoalModal} onClose={onModalClose} goal={form.getValues()} />
      </VStack>
    </CenterLayout>
  )
}
