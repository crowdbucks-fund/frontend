  ...
  return (
    <>
      ...
      {communities && communities.length === 0 && (
        <CreateFirstEntity mobileButtonText="Create Community" title="Start your journey now" btnText="Create your first community to start" link="/console/communities/create" />
      )}
      {communities && communities.length > 0 && <Communities communities={communities} />}
    </>
  )
}
    ...
    <CenterLayout maxW={{ md: '630px' }}>
      ...
      <VStack gap={{ base: 4, md: 6 }} w="full">
        {communities.map((community) => {
          return <CommunityCard key={community.id} community={community} href={`/console/communities/${community.id}`} />
        })}
